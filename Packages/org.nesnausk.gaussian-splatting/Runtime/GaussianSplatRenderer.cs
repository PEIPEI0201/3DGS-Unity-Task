...
namespace GaussianSplatting.Runtime
{
    ...
    public enum SplatVisualEffectMode
    {
        None = 0,
        DissolveSparkle = 1,
        WaterFlow = 3,
    ...
    [Serializable]
    public class DissolveEffectSettings
    {
        [Header("=== 细闪溶解 ===")]
        [Range(0f, 1f)]
        ...
        public DissolveMode dissolveMode = DissolveMode.DistanceFromCamera;
        public Vector3 dissolveDirection = Vector3.forward;
        [Range(1f, 80f)]
        public float sparkleDensity = 28f;
        [Range(0.15f, 0.9f)]
        public float sparkleCoverage = 0.62f;
        [Range(0.3f, 15f)]
        public float sparklePointSize = 2.5f;
        [Range(1.2f, 8f)]
        public float sparkleFlakeAspect = 3.2f;
        [Range(0f, 25f)]
        public float sparkleIntensity = 12f;
        [Range(0.5f, 30f)]
        public float sparkleSpeed = 8f;
        [Range(0.05f, 0.85f)]
        public float sparkleThreshold = 0.22f;
    }
    ...
    public class WaterFlowSettings
    {
        [Header("=== 水波流动 ===")]
        [Range(0.001f, 0.25f)]
        public float flowAmplitude = 0.045f;
        [Range(0.05f, 3f)]
        public float flowSpeed = 0.55f;
        [Range(0.2f, 10f)]
        public float flowFrequency = 1.6f;
    ...
    [Serializable]
    public class HologramScanSettings
    {
        [Header("=== 全息扫描 ===")]
        [Range(0.1f, 3f)]
        public float scanSpeed = 0.75f;
        [Range(0.3f, 8f)]
        public float scanWaveScale = 1.4f;
        [Range(0.04f, 0.5f)]
        public float scanBandWidth = 0.14f;
        public Vector3 scanDirection = Vector3.up;
        [Range(0f, 3f)]
        public float scanIntensity = 1.2f;
        [Range(0f, 1f)]
    ...
    public class GaussianSplatRenderer : MonoBehaviour
    {
        ...
        [SerializeField]
        SplatVisualEffectMode m_VisualEffectMode = SplatVisualEffectMode.DissolveSparkle;

        [SerializeField]
        DissolveEffectSettings m_DissolveEffects = new DissolveEffectSettings();

        [SerializeField]
        WaterFlowSettings m_WaterFlowEffects = new WaterFlowSettings();
        ...
        internal static class Props
        {
            ...
            public static readonly int VisualEffectMode = Shader.PropertyToID("_VisualEffectMode");
            ...
            public static readonly int DissolveBoundsMin = Shader.PropertyToID("_DissolveBoundsMin");
            public static readonly int DissolveBoundsMax = Shader.PropertyToID("_DissolveBoundsMax");
            public static readonly int SparkleDensity = Shader.PropertyToID("_SparkleDensity");
            public static readonly int SparkleCoverage = Shader.PropertyToID("_SparkleCoverage");
            public static readonly int SparklePointSize = Shader.PropertyToID("_SparklePointSize");
            public static readonly int SparkleFlakeAspect = Shader.PropertyToID("_SparkleFlakeAspect");
            ...
            public static readonly int SparkleThreshold = Shader.PropertyToID("_SparkleThreshold");
            public static readonly int WaterFlowAmplitude = Shader.PropertyToID("_WaterFlowAmplitude");
            public static readonly int WaterFlowSpeed = Shader.PropertyToID("_WaterFlowSpeed");
        ...
        void SetVisualEffectParamsOnCS(CommandBuffer cmb)
        {
            ComputeShader cs = m_CSSplatUtilities;
            ...
            cmb.SetComputeFloatParam(cs, Props.DissolveProgress, dissolve.dissolveProgress);
            cmb.SetComputeFloatParam(cs, Props.DissolveEdgeWidth, dissolve.dissolveEdgeWidth);
            cmb.SetComputeIntParam(cs, Props.DissolveMode, (int)dissolve.dissolveMode);
            cmb.SetComputeVectorParam(cs, Props.DissolveDirection, dissolve.dissolveDirection);
            cmb.SetComputeFloatParam(cs, Props.SparkleDensity, dissolve.sparkleDensity);
            cmb.SetComputeFloatParam(cs, Props.SparkleCoverage, dissolve.sparkleCoverage);
            cmb.SetComputeFloatParam(cs, Props.SparklePointSize, dissolve.sparklePointSize);
            cmb.SetComputeFloatParam(cs, Props.SparkleFlakeAspect, dissolve.sparkleFlakeAspect);
            cmb.SetComputeFloatParam(cs, Props.SparkleIntensity, dissolve.sparkleIntensity);
            cmb.SetComputeFloatParam(cs, Props.SparkleSpeed, dissolve.sparkleSpeed);
            cmb.SetComputeFloatParam(cs, Props.SparkleThreshold, dissolve.sparkleThreshold);

            if (HasValidAsset)
            {
                cmb.SetComputeVectorParam(cs, Props.DissolveBoundsMin, m_Asset.boundsMin);
        ...
        {
            if (m_VisualEffectMode == SplatVisualEffectMode.None)
                return;
            m_EffectTime = Time.realtimeSinceStartup;
        }
        ...
        {
            m_DissolveEffects.dissolveProgress = Mathf.Clamp01(progress);
        }

        public void PlayDissolve(float duration = 2f, bool reverse = false)
        {
            if (!Application.isPlaying)
                return;
            m_VisualEffectMode = SplatVisualEffectMode.DissolveSparkle;
            if (m_DissolveCoroutine != null)
                StopCoroutine(m_DissolveCoroutine);
            m_DissolveCoroutine = StartCoroutine(DissolveAnimation(duration, reverse));
        }

        public void PlayDissolve_Backup(float duration = 2f, bool reverse = false) => PlayDissolve(duration, reverse);
        ...
        System.Collections.IEnumerator DissolveAnimation(float duration, bool reverse)
        {
            ...
            {
                elapsed += Time.deltaTime;
                m_DissolveEffects.dissolveProgress = Mathf.Lerp(start, end, elapsed / duration);
                yield return null;
            }

            m_DissolveEffects.dissolveProgress = end;
            m_DissolveCoroutine = null;
        }
    }
}